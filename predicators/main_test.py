...
To run all RoboKitchen tests:
    python predicators/main.py --test_robo_kitchen_actions
    
    Note: For running tests, you only need the --test_robo_kitchen_actions flag.
    You do not need to provide --env, --approach, or --seed.

To run a specific RoboKitchen test:
    python predicators/main.py --test_robo_kitchen_actions --robo_kitchen_test <test_name>
    
    Valid test names are:
    - action_conversion: Tests the conversion between predicators actions and RoboCasa actions
    - action_space: Tests the action space of the RoboKitchen environment
    - task_generation: Tests the generation of training tasks for the RoboKitchen environment
"""
...
from predicators.approaches import ApproachFailure, ApproachTimeout, create_approach
...
from predicators.envs.robo_kitchen import (
    RoboKitchenEnv,
...
def run_robo_kitchen_tests(test_name=None):
    """Run all or a specific robo kitchen test.

    This function is a wrapper to run the tests from the test_robo_kitchen_actions.py file.
    These tests can also be run directly with pytest using:
    pytest predicators_robocasa/tests/robocasa_integration/test_robo_kitchen_actions.py -v

    Args:
        test_name (str, optional): Name of the specific test to run. If None, run all tests.
            Valid options: "action_conversion", "action_space", "task_generation"

    Returns:
        bool: True if all executed tests passed, False otherwise.
    """
    if test_name:
        logging.info(f"\n=== Running RoboKitchen Test: {test_name} ===\n")
    else:
        logging.info("\n=== Running All RoboKitchen Tests ===\n")

    # Set seed for reproducibility (these should already be set by main(), but set as fallback)
    if not hasattr(CFG, "seed"):
        CFG.seed = 0
    if not hasattr(CFG, "num_test_tasks"):
        CFG.num_test_tasks = 1
    if not hasattr(CFG, "robo_kitchen_randomize_init_state"):
        CFG.robo_kitchen_randomize_init_state = True

    # Import test functions from test file to avoid duplication
    try:
        ...
        from tests.robocasa_integration.test_robo_kitchen_actions import (
            test_action_conversion,
            test_action_space,
            test_robo_kitchen_training_task_generation,
        )

        # Track overall test results
        all_passed = True

        # Run test_action_conversion if requested or running all tests
        if test_name is None or test_name == "action_conversion":
            logging.info("Running test_action_conversion...")
            action_conv_passed = True
            try:
                test_action_conversion()
                logging.info("✓ test_action_conversion passed")
            except Exception as e:
                logging.error(f"✗ test_action_conversion failed: {e}")
                action_conv_passed = False
                all_passed = False

        # Run test_action_space if requested or running all tests
        if test_name is None or test_name == "action_space":
            logging.info("\nRunning test_action_space...")
            action_space_passed = True
            try:
                test_action_space()
                logging.info("✓ test_action_space passed")
            except Exception as e:
                logging.error(f"✗ test_action_space failed: {e}")
                action_space_passed = False
                all_passed = False

        # Run test_robo_kitchen_training_task_generation if requested or running all tests
        if test_name is None or test_name == "task_generation":
            logging.info("\nRunning test_robo_kitchen_training_task_generation...")
            task_gen_passed = True
            try:
                test_robo_kitchen_training_task_generation()
                logging.info("✓ test_robo_kitchen_training_task_generation passed")
            except Exception as e:
                logging.error(
                    f"✗ test_robo_kitchen_training_task_generation failed: {e}"
                )
                task_gen_passed = False
                all_passed = False

        # Check if the requested test exists
        if test_name and test_name not in [
            "action_conversion",
            "action_space",
            "task_generation",
        ]:
            logging.error(f"Unknown test name: {test_name}")
            logging.error(
                "Valid test names are: 'action_conversion', 'action_space', 'task_generation'"
            )
            return False

        if all_passed:
            if test_name:
                logging.info(f"\n=== RoboKitchen Test '{test_name}' Passed! ===\n")
            else:
                logging.info("\n=== All RoboKitchen Tests Passed! ===\n")
        else:
            if test_name:
                logging.error(f"\n=== RoboKitchen Test '{test_name}' Failed! ===\n")
            else:
                logging.error("\n=== Some RoboKitchen Tests Failed! ===\n")

        return all_passed

    except ImportError as e:
        logging.error(f"Failed to import test functions: {e}")
        logging.error(
            "Make sure the test file exists at 'tests/robocasa_integration/test_robo_kitchen_actions.py'"
        )
        return False
...
def main() -> None:
    ...
    # Parse args first to check for test_robo_kitchen_actions flag
    args = utils.parse_args(
        env_required=False, approach_required=False, seed_required=False
    )
    utils.update_config(args)

    # Check for test_robo_kitchen_actions flag
    if hasattr(CFG, "test_robo_kitchen_actions") and CFG.test_robo_kitchen_actions:
        ...
        # Get the specific test to run if provided
        test_name = None
        if hasattr(CFG, "robo_kitchen_test") and CFG.robo_kitchen_test:
            test_name = CFG.robo_kitchen_test

        # Run tests
        run_robo_kitchen_tests(test_name)
        return

    # Normal main.py execution for non-test scenarios
...
def _run_pipeline(
    ...
    offline_dataset: Optional[Dataset] = None,
) -> None:
    ...
    if cogman.is_learning_based:
        ...
        offline_learning_metrics = {
            f"offline_learning_{k}": v for k, v in cogman.metrics.items()
        }
...
def _generate_interaction_results(
    ...
    for request in requests:
        if (
            request.train_task_idx < CFG.max_initial_demos
            and not CFG.allow_interaction_in_demo_tasks
        ):
            raise RuntimeError(
                "Interaction requests cannot be on demo tasks "
        ...
        request_responses: List[Optional[Response]] = [None for _ in traj.states]
        ...
        result = InteractionResult(traj.states, traj.actions, request_responses)
...
def _run_testing(env: BaseEnv, cogman: CogMan) -> Metrics:
    ...
    for test_task_idx, env_task in enumerate(test_tasks):
        ...
        metrics[f"PER_TASK_task{test_task_idx}_nodes_created"] = (
            cogman.metrics["total_num_nodes_created"] - curr_num_nodes_created
        )
        metrics[f"PER_TASK_task{test_task_idx}_nodes_expanded"] = (
        ...
        try:
            ...
            log_message = "Approach failed at policy execution time with " f"error: {e}"
            if isinstance(e, ApproachTimeout):
        ...
        logging.info(f"Task {test_task_idx+1} / {len(test_tasks)}: " f"{log_message}")
        if make_video:
            assert monitor is not None
...
def _save_test_results(results: Metrics, online_learning_cycle: Optional[int]) -> None:
    ...
    outfile = (
        f"{CFG.results_dir}/{utils.get_config_path_str()}__"
