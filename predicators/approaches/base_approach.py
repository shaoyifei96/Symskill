...
class BaseApproach(abc.ABC):
    ...
    def solve(self, task: Task, timeout: int, stay_close_to_previous_plan: bool = False, last_option_name: str = None) -> Callable[[State], Action]:
        """Light wrapper around the abstract self._solve().

        Checks that actions are in the action space.
        """
        pi = self._solve(task, timeout, stay_close_to_previous_plan, last_option_name)
