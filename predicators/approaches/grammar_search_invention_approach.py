...
from predicators.predicate_search_score_functions import _PredicateSearchScoreFunction, create_score_function
from predicators.settings import CFG
from predicators.structs import Dataset, GroundAtom, GroundAtomTrajectory, Object, ParameterizedOption, Predicate, Segment, State, Task, Type, VLMPredicate
...
def _create_grammar(dataset: Dataset, given_predicates: Set[Predicate]) -> _PredicateGrammar:
    ...
    if CFG.grammar_search_use_trans_quat_features:
        ...
        grammar: _PredicateGrammar = _SingleFeatureInequalitiesPredicateGrammar(dataset)

        if CFG.grammar_search_grammar_use_diff_features:
            diff_grammar = _FeatureDiffInequalitiesPredicateGrammar(dataset)
            grammar = _ChainPredicateGrammar([grammar, diff_grammar], alternate=True)
        if CFG.grammar_search_grammar_use_euclidean_dist:
            for t1_f1, t1_f2, t2_f1, t2_f2 in CFG.grammar_search_euclidean_feature_names:
                euclidean_dist_grammar = _EuclideanDistancePredicateGrammar(dataset, t1_f1, t2_f1, t1_f2, t2_f2)
    ...
    # predicates.
    if not CFG.grammar_search_use_handcoded_debug_grammar and CFG.grammar_search_prune_redundant_preds:
...
class _TransComponentCompareClassifier(_BinaryClassifier):
    ...
    object2_index: int
    object2_type: Type
    attribute_name: str  # "translation"
    ...
    def _classify_object(self, s: State, obj1: Object, obj2: Object) -> bool:
        ...
        return self.compare(abs(trans1[self.component_idx] - trans2[self.component_idx]), self.constant)

    def __str__(self) -> str:
        component = self.componets[self.component_idx]
        return (
            f"(|({self.object1_index}:{self.object1_type.name}).{self.attribute_name}[{component}] - "
            f"({self.object2_index}:{self.object2_type.name}).{self.attribute_name}[{component}]|{self.compare_str}"
    ...
    def pretty_str(self) -> Tuple[str, str]:
        ...
        body_str = f"(|{name1}.{self.attribute_name}[{component}] - " f"{name2}.{self.attribute_name}[{component}]| " f"{self.compare_str} {self.constant:.3})"
...
class _RotationAlignmentClassifier(_BinaryClassifier):
    ...
    def __str__(self) -> str:
        return (
            f"(angle_diff(({self.object1_index}:{self.object1_type.name}).{self.attribute_name}, "
            f"({self.object2_index}:{self.object2_type.name}).{self.attribute_name}){self.compare_str}"
            f"[idx {self.constant_idx}]{self.constant:.3})"
        )
...
class _SingleAttributeCompareClassifier(_UnaryClassifier):
    ...
    def __str__(self) -> str:
        return f"(({self.object_index}:{self.object_type.name})." f"{self.attribute_name}{self.compare_str}[idx {self.constant_idx}]" f"{self.constant:.3})"

    def pretty_str(self) -> Tuple[str, str]:
        name = CFG.grammar_search_classifier_pretty_str_names[self.object_index]
        vars_str = f"{name}:{self.object_type.name}"
        body_str = f"({name}.{self.attribute_name} " f"{self.compare_str} {self.constant:.3})"
        return vars_str, body_str
...
class _AttributeDiffCompareClassifier(_BinaryClassifier):
    ...
    def pretty_str(self) -> Tuple[str, str]:
        name1 = CFG.grammar_search_classifier_pretty_str_names[self.object1_index]
        name2 = CFG.grammar_search_classifier_pretty_str_names[self.object2_index]
        vars_str = f"{name1}:{self.object1_type.name}, " f"{name2}:{self.object2_type.name}"
        body_str = f"(|{name1}.{self.attribute1_name} - " f"{name2}.{self.attribute2_name}| " f"{self.compare_str} {self.constant:.3})"
        return vars_str, body_str
...
class _EuclideanAttributeDiffCompareClassifier(_BinaryClassifier):
    ...
    def _classify_object(self, s: State, obj1: Object, obj2: Object) -> bool:
        assert obj1.type == self.object1_type
        assert obj2.type == self.object2_type
        return self.compare((s.get(obj1, self.obj1_attr1_name) - s.get(obj2, self.obj2_attr1_name)) ** 2 + (s.get(obj1, self.obj1_attr2_name) - s.get(obj2, self.obj2_attr2_name)) ** 2, self.constant)
...
class _ForallClassifier(_NullaryClassifier):
    ...
    def pretty_str(self) -> Tuple[str, str]:
        ...
        head = ", ".join(f"{CFG.grammar_search_classifier_pretty_str_names[i]}:{t.name}" for i, t in enumerate(types))
...
class _UnaryFreeForallClassifier(_UnaryClassifier):
    ...
    def _quantified_types(self) -> List[Type]:
        return [t for i, t in enumerate(self.body.types) if i != self.free_variable_idx]
    ...
    def __str__(self) -> str:
        types = self.body.types
        type_sig = ",".join(f"{i}:{t.name}" for i, t in enumerate(types) if i != self.free_variable_idx)
        objs = ",".join(str(i) for i in range(len(types)))
        return f"Forall[{type_sig}].[{str(self.body)}({objs})]"
    ...
    def pretty_str(self) -> Tuple[str, str]:
        types = self.body.types
        _, body_str = self.body.pretty_str()
        head = ", ".join(f"{CFG.grammar_search_classifier_pretty_str_names[i]}:{t.name}" for i, t in enumerate(types) if i != self.free_variable_idx)
        name = CFG.grammar_search_classifier_pretty_str_names[self.free_variable_idx]
...
_DEBUG_GEOMETRIC_PREDICATES = {
    ...
    "burger": ["((0:robot).fingers<=[idx 0]0.5)"],
    "burger_no_move": [
        "((0:robot).fingers<=[idx 0]0.5)",
    ],
    "unittest": ["((0:robot).hand<=[idx 0]0.65)", "((0:block).grasp<=[idx 0]0.0)", "NOT-Forall[0:block].[((0:block).width<=[idx 0]0.085)(0)]"],
}
...
class _DebugGrammar(_PredicateGrammar):
    ...
    def generate(self, max_num: int) -> Dict[Predicate, float]:
        del max_num
        env_name = CFG.env if not CFG.env.startswith("pybullet") else CFG.env[CFG.env.index("_") + 1 :]
    ...
    def enumerate(self) -> Iterator[Tuple[Predicate, float]]:
        ...
        for predicate, cost in self.base_grammar.enumerate():
            if any(str(predicate).startswith(debug_str) for debug_str in _DEBUG_GEOMETRIC_PREDICATES[env_name]):
...
def _halving_constant_generator(lo: float, hi: float, cost: float = 1.0) -> Iterator[Tuple[float, float]]:
        ...
        for constant_idx, (constant, _) in enumerate(constant_generator):
            for type1, type2 in itertools.combinations_with_replacement(sorted(self.types), 2):
                ...
                for i in range(len(self.components)):
                    lb1, ub1 = feature_ranges[type1][self.components[i]]
                    lb2, ub2 = feature_ranges[type2][self.components[i]]
                    if abs(lb1 - ub1) < 1e-6 or abs(lb2 - ub2) < 1e-6:
                        continue
            ...
            for state in traj.states:
                for obj in state:
                    if obj.type not in feature_ranges:
                        ...
                        for i in range(len(self.components)):
                            feature_ranges[obj.type][self.components[i]] = (v[i], v[i])
                    else:
                        v = state.get(obj, self.attribute_name)
                        for i in range(len(self.components)):
                            mn, mx = feature_ranges[obj.type][self.components[i]]
                            feature_ranges[obj.type][self.components[i]] = (min(mn, v[i]), max(mx, v[i]))
        ...
        for constant_idx, (constant, cost) in enumerate(constant_generator):
            for t in sorted(self.types):
                for f in t.feature_names:
                    ...
                    comp, comp_str = le, "<="
                    classifier = _SingleAttributeCompareClassifier(0, t, f, k, constant_idx, comp, comp_str)
                    name = str(classifier)
                    types = [t]
                    pred = Predicate(name, types, classifier)
                    assert pred.arity == 1
                ...
                for obj in state:
                    if obj.type not in feature_ranges:
                        ...
                        for f in obj.type.feature_names:
                            ...
                            v = state.get(obj, f)
                            feature_ranges[obj.type][f] = (min(mn, v), max(mx, v))
        return feature_ranges


@dataclass(frozen=True, eq=False, repr=False)
class _FeatureDiffInequalitiesPredicateGrammar(_SingleFeatureInequalitiesPredicateGrammar):
    """Generates features of the form "|0.feature - 1.feature| <= c"."""

    def _yield_pred_given_const(self, feature_ranges: Dict[Type, Dict[str, Tuple[float, float]]], constant_idx: int, constant: float, cost: float) -> Iterator[Tuple[Predicate, float]]:
        for t1, t2 in itertools.combinations_with_replacement(sorted(self.types), 2):
            for f1 in t1.feature_names:
                for f2 in t2.feature_names:
                    ...
                    # Create classifier.
                    comp, comp_str = le, "<="
                    diff_classifier = _AttributeDiffCompareClassifier(0, t1, f1, 1, t2, f2, k, constant_idx, comp, comp_str)
    ...
    def enumerate(self) -> Iterator[Tuple[Predicate, float]]:
        ...
        for ret_val in self._yield_pred_given_const(feature_ranges, 0, CFG.grammar_search_diff_features_const_multiplier, 4.0):
            yield ret_val
        # 0.5, 0.25, 0.75, 0.125, 0.375, ...
        constant_generator = _halving_constant_generator(0.0, 1.0)
        for constant_idx, (constant, cost) in enumerate(constant_generator):
            for ret_val in self._yield_pred_given_const(feature_ranges, constant_idx, constant, cost):
                yield ret_val


@dataclass(frozen=True, eq=False, repr=False)
class _EuclideanDistancePredicateGrammar(_SingleFeatureInequalitiesPredicateGrammar):
    ...
    def _compute_xy_bounds(self, feature_ranges: Dict[Type, Dict[str, Tuple[float, float]]], t1: Type, t2: Type) -> Tuple[float, float]:
        # To create our classifier, we need to leverage the
        ...
        lbx, ubx = utils.compute_abs_range_given_two_ranges(lbx1, ubx1, lbx2, ubx2)
        lby, uby = utils.compute_abs_range_given_two_ranges(lby1, uby1, lby2, uby2)
    ...
    def _generate_pred_given_constant(self, constant_idx: int, constant: float, t1: Type, t2: Type, t1_f1_name: str, t1_f2_name: str, t2_f1_name: str, t2_f2_name: str) -> Predicate:
        # Create classifier.
        comp, comp_str = le, "<="
        diff_classifier = _EuclideanAttributeDiffCompareClassifier(0, t1, t1_f1_name, t1_f2_name, 1, t2, t2_f1_name, t2_f2_name, constant, constant_idx, comp, comp_str)
        name = str(diff_classifier)
        types = [t1, t2]
        pred = Predicate(name, types, diff_classifier)
        return pred
    ...
    def enumerate(self) -> Iterator[Tuple[Predicate, float]]:
        ...
        for t1, t2 in itertools.combinations_with_replacement(sorted(self.types), 2):
            if self.t1_f1_name in t1.feature_names and self.t2_f1_name in t2.feature_names and self.t1_f2_name in t1.feature_names and self.t2_f2_name in t2.feature_names:
                lb, ub = self._compute_xy_bounds(feature_ranges, t1, t2)
                constant = ((ub - lb) * CFG.grammar_search_euclidean_const_multiplier) + lb
                pred = self._generate_pred_given_constant(0, constant, t1, t2, self.t1_f1_name, self.t1_f2_name, self.t2_f1_name, self.t2_f2_name)
                assert pred.arity == 2
        ...
        for constant_idx, (constant, cost) in enumerate(constant_generator):
            for t1, t2 in itertools.combinations_with_replacement(sorted(self.types), 2):
                if self.t1_f1_name in t1.feature_names and self.t2_f1_name in t2.feature_names and self.t1_f2_name in t1.feature_names and self.t2_f2_name in t2.feature_names:
                    ...
                    # Scale the constant by the correct range.
                    k = constant * (ub - lb) + lb
                    pred = self._generate_pred_given_constant(constant_idx + 1, k, t1, t2, self.t1_f1_name, self.t1_f2_name, self.t2_f1_name, self.t2_f2_name)
                    assert pred.arity == 2
...
class _ChainPredicateGrammar(_PredicateGrammar):
    ...
    def enumerate(self) -> Iterator[Tuple[Predicate, float]]:
        if not self.alternate:
            return itertools.chain.from_iterable(g.enumerate() for g in self.base_grammars)
...
class _PrunedGrammar(_DataBasedPredicateGrammar):
    ...
    base_grammar: _PredicateGrammar
    _state_sequences: List[List[State]] = field(init=False, default_factory=list)
    ...
    def enumerate(self) -> Iterator[Tuple[Predicate, float]]:
        ...
        seen: Dict[FrozenSet[Tuple[int, int, FrozenSet[Tuple[Object, ...]]]], Predicate] = {}  # keys are from _get_predicate_identifier()
        for predicate, cost in self.base_grammar.enumerate():
            if cost >= CFG.grammar_search_predicate_cost_upper_bound:
                return
            pred_id = self._get_predicate_identifier(predicate)
            if pred_id in seen:
                logging.debug(f"Pruning {predicate} b/c equal to " f"{seen[pred_id]}")
                continue
            # Found a new predicate.
            seen[pred_id] = predicate
            yield (predicate, cost)

    def _get_predicate_identifier(self, predicate: Predicate) -> FrozenSet[Tuple[int, int, FrozenSet[Tuple[Object, ...]]]]:
        """Returns frozenset identifiers for each data point."""
        raw_identifiers = set()
        if CFG.segmenter == "atom_changes":
            # Get atoms for this predicate alone on the dataset, and then
            # go through the entire dataset.
            atom_dataset = utils.create_ground_atom_dataset(self.dataset.trajectories, {predicate})
...
class _NegationPredicateGrammarWrapper(_PredicateGrammar):
    ...
    def enumerate(self) -> Iterator[Tuple[Predicate, float]]:
        for predicate, cost in self.base_grammar.enumerate():
            ...
            negated_predicate = Predicate(str(classifier), predicate.types, classifier)
...
class _ForallPredicateGrammarWrapper(_PredicateGrammar):
    ...
    def enumerate(self) -> Iterator[Tuple[Predicate, float]]:
        ...
        for predicate, cost in self.base_grammar.enumerate():
            ...
            forall_predicate = Predicate(str(forall_classifier), [], forall_classifier)
            ...
            notforall_predicate = Predicate(str(notforall_classifier), forall_predicate.types, notforall_classifier)
            ...
            if predicate.arity >= 2:
                for idx in range(predicate.arity):
                    ...
                    uff_predicate = Predicate(str(uff_classifier), [predicate.types[idx]], uff_classifier)
                    assert uff_predicate.arity == 1
                    yield (uff_predicate, cost + forall_penalty + 1)
                    # Negated UFF
                    notuff_classifier = _NegationClassifier(uff_predicate)
                    notuff_predicate = Predicate(str(notuff_classifier), uff_predicate.types, notuff_classifier)
...
class GrammarSearchInventionApproach(NSRTLearningApproach):
    ...
    def __init__(self, initial_predicates: Set[Predicate], initial_options: Set[ParameterizedOption], types: Set[Type], action_space: Box, train_tasks: List[Task]) -> None:
    ...
    def _generate_atom_dataset_via_grammar(self, dataset: Dataset) -> Tuple[List[GroundAtomTrajectory], Dict[Predicate, float]]:
        ...
        if CFG.load_atoms:
            atom_dataset = utils.load_ground_atom_dataset(dataset_fname, dataset.trajectories)
        else:
            atom_dataset = utils.create_ground_atom_dataset(dataset.trajectories, set(renamed_candidates) | self._initial_predicates)
    ...
    def _parse_atom_dataset_from_annotated_dataset(self, dataset: Dataset) -> Tuple[List[GroundAtomTrajectory], Dict[Predicate, float]]:
            ...
            for ground_atom_state in ano_traj:
                for ground_atom in ground_atom_state:
                    ...
                    if ground_atom.predicate not in candidates:
                        ...
                        candidates[ground_atom.predicate] = float(len(ground_atom.objects))
    ...
    def _rename_predicates_to_remove_incompatible_chars(self, predicates_and_costs: Dict[Predicate, float]) -> Dict[Predicate, float]:
        ...
        for p in predicates_and_costs:
            if isinstance(p, VLMPredicate):  # pragma: no cover.
                ...
                continue
            new_name = p.name.replace("(", "[").replace(")", "]").replace(" ", "_")
    ...
    def learn_from_offline_dataset(self, dataset: Dataset) -> None:
        if CFG.offline_data_method in ["geo_and_demo_with_vlm_imgs", "geo_and_demo+labelled_atoms", "geo_and_saved_vlm_img_demos_folder"]:
            atom_dataset_from_grammar, candidates_from_grammar = self._generate_atom_dataset_via_grammar(dataset)
            atom_dataset_from_vlm, candidates_from_vlm = self._parse_atom_dataset_from_annotated_dataset(dataset)
            atom_dataset = utils.merge_ground_atom_datasets(atom_dataset_from_grammar, atom_dataset_from_vlm)
        ...
        elif not CFG.offline_data_method in ["demo+labelled_atoms", "saved_vlm_img_demos_folder", "demo_with_vlm_imgs"]:
            atom_dataset, candidates = self._generate_atom_dataset_via_grammar(dataset)
        else:
            # In this case, we're inventing over already-labelled atoms.
            atom_dataset, candidates = self._parse_atom_dataset_from_annotated_dataset(dataset)
        # Select a subset of the candidates to keep.
        logging.info("Selecting a subset...")
        if CFG.grammar_search_pred_selection_approach == "score_optimization":
            if CFG.grammar_search_select_all_debug and CFG.grammar_search_use_handcoded_debug_grammar:
                ...
                debug_predicate_names = _DEBUG_VLM_PREDICATES[CFG.env] + renamed_debug_geometric_predicates
                self._learned_predicates = set(p for p in candidates.keys() if p.name in debug_predicate_names)
            else:
                # Create the score function that will be used to guide search.
                score_function = create_score_function(CFG.grammar_search_score_function, self._initial_predicates, atom_dataset, candidates, self._train_tasks)
                self._learned_predicates = self._select_predicates_by_score_hillclimbing(candidates, score_function, self._initial_predicates, atom_dataset, self._train_tasks)
        elif CFG.grammar_search_pred_selection_approach == "clustering":
            self._learned_predicates = self._select_predicates_by_clustering(candidates, self._initial_predicates, dataset, atom_dataset)
        ...
        self._learn_nsrts(dataset.trajectories, online_learning_cycle=None, annotations=annotations)

    def _select_predicates_by_score_hillclimbing(
        self, candidates: Dict[Predicate, float], score_function: _PredicateSearchScoreFunction, initial_predicates: Set[Predicate], atom_dataset: List[GroundAtomTrajectory], train_tasks: List[Task]
    ) -> Set[Predicate]:
        ...
        def _get_successors(s: FrozenSet[Predicate]) -> Iterator[Tuple[None, FrozenSet[Predicate], float]]:
        ...
        if CFG.grammar_search_search_algorithm == "hill_climbing":
            ...
            for i in range(1, len(path)):  # pragma: no cover
                ...
                logging.info(f"\tOn step {i}, added {new_addition}, with " f"heuristic {h:.3f} (an improvement of " f"{prev_h - h:.3f} over the previous step)")
        elif CFG.grammar_search_search_algorithm == "gbfs":
            path, _ = utils.run_gbfs(init, _check_goal, _get_successors, score_function.evaluate, max_evals=CFG.grammar_search_gbfs_num_evals)
        else:
            raise NotImplementedError("Unrecognized grammar_search_search_algorithm: " f"{CFG.grammar_search_search_algorithm}.")
        ...
        self._metrics["total_num_predicate_evaluations"] = len(path) * len(candidates)
        ...
        segmented_trajs = [segment_trajectory(ll_traj, set(preds), atom_seq=atom_seq) for (ll_traj, atom_seq) in pruned_atom_data]
        low_level_trajs = [ll_traj for ll_traj, _ in pruned_atom_data]
        preds_in_preconds = set()
        for pnad in learn_strips_operators(low_level_trajs, train_tasks, set(kept_predicates | initial_predicates), segmented_trajs, verify_harmlessness=False, annotations=None, verbose=False):
            for atom in pnad.op.preconditions:
                preds_in_preconds.add(atom.predicate)
        kept_predicates &= preds_in_preconds

        logging.info(f"\nSelected {len(kept_predicates)} predicates out of " f"{len(candidates)} candidates:")
    ...
    def _select_predicates_by_clustering(self, candidates: Dict[Predicate, float], initial_predicates: Set[Predicate], dataset: Dataset, atom_dataset: List[GroundAtomTrajectory]) -> Set[Predicate]:
        ...
        if CFG.grammar_search_pred_clusterer == "option-type-number-sample":
            ...
            segments = [seg for ll_traj, atom_seq in atom_dataset for seg in segment_trajectory(ll_traj, initial_predicates, atom_seq)]
            ...
            logging.info(f"STEP 1: generated {len(option_to_segments.keys())} " f"option-based clusters.")
            ...
            for i, pair in enumerate(option_to_segments.items()):
                ...
                for seg in segments:
                    ...
                    assert len(types_in_effects) > 0 and len(set.union(*types_in_effects)) > 0
                    types = tuple(sorted(list(set.union(*types_in_effects))))
                    types_to_segments.setdefault(types, []).append(seg)
                logging.info(f"STEP 2: generated {len(types_to_segments.keys())} " f"type-based clusters for cluster {i+1} from STEP 1 " f"involving option {option}.")
            ...
            for i, (option, types_to_segments) in enumerate(clusters.items()):
                for j, (types, segments) in enumerate(types_to_segments.items()):
                    ...
                    logging.info(f"STEP 3: generated {len(num_to_segments.keys())} " f"num-object-based clusters for cluster {i+j+1} from " f"STEP 2 involving option {option} and type {types}.")
            ...
            for i in range(num_clusters):
                for j in range(num_clusters):
                    ...
                    if consistent_shared_add_effects_per_cluster[i] == consistent_shared_add_effects_per_cluster[j]:
                        logging.info(f"Final clusters {i} and {j} cannot be " f"disambiguated after removing the inconsistent" f" predicates.")
            ...
            logging.info(f"\nSelected {len(predicates_to_keep)} predicates out of " f"{len(candidates)} candidates:")
        ...
        if CFG.grammar_search_pred_clusterer == "oracle":
            ...
            assert dataset.annotations is not None and len(dataset.annotations) == len(dataset.trajectories)
            assert CFG.segmenter == "option_changes"
            preds = set(initial_predicates) | initial_predicates
            segmented_trajs = [segment_trajectory(ll_traj, preds, atom_seq) for ll_traj, atom_seq in atom_dataset]
            assert len(segmented_trajs) == len(dataset.annotations)
            # First, get the set of all ground truth operator names.
            all_gt_ops = set(ground_nsrt.parent for anno_list in dataset.annotations for ground_nsrt in anno_list)
            # Next, make a dictionary mapping operator name to segments
            # where that operator was used.
            gt_op_to_segments: Dict[str, List[Segment]] = {op_name: [] for op_name in all_gt_ops}
        ...
        raise NotImplementedError("Unrecognized clusterer for predicate " + f"invention {CFG.grammar_search_pred_clusterer}.")
