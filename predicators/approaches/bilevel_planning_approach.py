...
class BilevelPlanningApproach(BaseApproach):
    ...
    def get_execution_monitoring_info(self) -> List[Tuple[Set[GroundAtom], Set[GroundAtom]]]:
        """Returns execution monitoring information as a list of (expected atoms, maintain effects) tuples.
        
        When planning without simulation, this returns the expected atoms and maintain effects
        for each step in the plan. Otherwise, returns an empty list.
        """
        if self._plan_without_sim:
            # Package expected atoms and maintain effects together
            # they are different length, last_atoms_seq is longer by 1
            # padding last_maintain_effects with empty set above
            return list(zip(self._last_atoms_seq, self._last_maintain_effects))
        return []
